...
def iniciar_chat_como_host():
    ...
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Crea un socket utilizando TCP
    ...
    hilo_recibir = threading.Thread(target=recv_msg, args=(conn,))
    hilo_recibir.start()

    hilo_enviar = threading.Thread(target=send_msg, args=(conn,))
    hilo_enviar.start()
...
def iniciar_chat_como_cliente():
    ...
    # Crea dos hilos: uno para recibir mensajes y otro para enviar mensajes a través del socket
    hilo_recibir = threading.Thread(target=recv_msg, args=(sock,))
    hilo_recibir.start()

    hilo_enviar = threading.Thread(target=send_msg, args=(sock,))
    hilo_enviar.start()
